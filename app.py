...
def whatsapp_webhook():
    ...
    if 'turno' in msg:
        return responder_whatsapp(
            "¿Preferís turno en una de nuestras sedes o necesitás atención a domicilio?"
        )
    ...
    if pacientes[tel].get('estado') == 'esperando_informes':
        ...
        if len(partes) >= 2:
            ...
            return responder_whatsapp(
                f"Solicitamos informes para {nombre.title()} en {localidad}. "
                "La sede correspondiente los recibirá y te los enviará."
            )
        return responder_whatsapp(
