...
def crear_hoja_del_dia(dia):
    ...
    folder_id = None
    try:
        results = drive_service.files().list(q="mimeType='application/vnd.google-apps.folder' and name='ALIA_TURNOS' and trashed=false", spaces='drive', fields='files(id)').execute()
...
def whatsapp_webhook():
    ...
    if any(k in msg for k in ['hola', 'buenas']):
        return responder_whatsapp("Hola! Soy ALIA, tu asistente de laboratorio. Escribí *ASISTENTE* en cualquier momento y te derivaré a un operador humano.")
    ...
    if 'sede' in msg and pacientes[tel]['estado'] is None:
        ...
        return responder_whatsapp("Perfecto. Para *SEDE*, escribí: Nombre, Localidad, Fecha de nacimiento (dd/mm/aaaa), Cobertura, N° Afiliado. *Separalos por comas*.")
    ...
    if any(k in msg for k in ['domicilio', 'casa']) and pacientes[tel]['estado'] is None:
        ...
        return responder_whatsapp("Perfecto. Para *DOMICILIO*, escribí: Nombre, Dirección, Localidad, Fecha de nacimiento (dd/mm/aaaa), Cobertura, N° Afiliado. *Separalos por comas*.")
    ...
    if pacientes[tel]['estado'] == 'esperando_datos':
        ...
        if len(partes) >= 6:
            ...
            return responder_whatsapp(f"Tu turno fue agendado para el día *{dia}* entre las 08:00 y 11:00 hs. Ahora, por favor, enviá la orden médica en una *foto JPG o PNG*.")
    ...
    if pacientes[tel]['estado'] == 'esperando_orden' and request.form.get('NumMedia') == '1':
        ...
        if 'pdf' in ctype:
            return responder_whatsapp("No puedo procesar PDF. Por favor, convertí tu orden médica a imagen JPG o PNG.")
        ...
        return responder_whatsapp(f"Detectamos estos estudios:\n{res}\n¿Son correctos? Respondé *Sí* o *No*.")
    ...
    prompt_fb = (
        ...
        f"OCR: {texto}\n"
        f"Pregunta: {body}\n"
        f"Responde únicamente si debe hacer ayuno (cuántas horas) o recolectar orina."
    )
    try:
        ...
        mensaje = "Ocurrió un error al procesar tu consulta. Intentá nuevamente más tarde."

    mensaje += "\n\n¡Gracias por comunicarte con ALIA! Escribí *ASISTENTE* en cualquier momento para ser derivado a un operador."
