...
def responder_whatsapp(texto): """Envía solo un mensaje de texto (sin encuesta).""" resp = MessagingResponse() resp.message(texto) return Response(str(resp), mimetype='application/xml')

def responder_encuesta_y_orden(texto): """Envía mensaje, encuesta y luego pide foto de orden médica.""" resp = MessagingResponse() resp.message(texto) encuesta = ( "¡Gracias por comunicarte con ALIA! " "Ayudanos a mejorar completando esta encuesta: " "https://forms.gle/gHPbyMJfF18qYuUq9" ) resp.message(encuesta) resp.message("Ahora por favor envía la orden médica en foto JPG/PNG.") return Response(str(resp), mimetype='application/xml')
...
if tel not in pacientes:
    pacientes[tel] = {'estado': None, 'reintentos': 0, 'ocr_fallos': 0, 'tipo_atencion': None}
    ...
    if len(partes) >= 6:
        ...
        if pacientes[tel]['tipo_atencion']=='DOMICILIO':
            ...
            hoja.append_row([
                ...
                direccion, localidad, fecha_nac, cobertura, afiliado, '', 'Pendiente'
            ])
            return responder_encuesta_y_orden(
