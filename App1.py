...
def determinar_localidad_sede(sede):
    sede = localidad.lower()
    if "ituzaingó" in localidad:
        return "ituzaingó"
    elif "merlo" in localidad or "padua" in localidad:
        return "Merlo" if datetime.today().weekday() < 4 else "Merlo"
    elif "tesei" in localidad or "hurlingham" in localidad:
        return "tesei" if datetime.today().weekday() < 4 else "tesei"
    elif "castelar" in localidad:
        return "castelar"
    else:
        return "castelar"
...
def whatsapp_webhook():
    ...
    if pacientes[telefono].get("estado") == "esperando_datos":
        ...
        if len(partes) >= 6:
            ...
            localidad_sede = determinar_localidad_sede(partes[2])
            pacientes[telefono]["localidad"] = localidad_sede
            hoja = crear_hoja_de_la_sede(localidad_sede)
            hoja.append_row([
                str(datetime.now()),
                partes[0].strip(),
                telefono,
            ...
            return responder_whatsapp(f"¡Gracias {partes[0].strip()}! Agendamos tus datos para ka sede {localidad_sede} entre las 08:00 y las 11:00 hs. ¿Podés enviarnos una foto de la orden médica?")
        else:
            return responder_whatsapp("Faltan datos. Por favor escribí: Nombre, Dirección, Localidad, Fecha de nacimiento, Cobertura y Afiliado (todo separado por comas).")

    if [telefono].get("estado") == "esperando_orden" and request.form.get("NumMedia") == "1":
